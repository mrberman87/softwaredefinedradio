...
def run(pid)
	if pid == 0:
		tb = txrx_controller(fc=440e6, centoff=11.19e3, foffset_tx=0, foffset_rx=50e3)
		time.sleep(1)
		...
		while True:
			...
			if s.count(':') > 0:
				...
				elif s[0] == 'frame_time_out':
					tb.set_frame_time_out(s[1])
				elif s[0] == 'set_frequency':
					tb.set_frequency(s[1], s[2])
				if len(s) == 3:
					if s[2] == 'rx':
		...
		class ground_controls(abstractmodel.AbstractModel, threading.Thread):
			...
			def set_params(self):
				...
				os.write(toServer, 'frame_time_out:' + self.timeout)
				os.write(toServer, 'set_frequency:' + str(self.freq) + ':' + str(self.calc_offset()))
			...
			def keep_alive(self):
				os.write(toServer, 'frame_time_out:' + '3')
