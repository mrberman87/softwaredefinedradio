...
class watch_dog1(Daemon):
	def run(self):
		...
		time.sleep(2)
		wd = wd_reset(self.working_dir + '/daemon_pids/wd2_wd1.wd', 5).start()
		while True:
			time.sleep(7)
			...
			if not self.check_file(self.working_dir + '/daemon_pids/wd2_controller.wd'):
				os.system('python %s/uav_controller.py restart'%self.working_dir)
			
			if self.pid_exists(self.working_dir + '/daemon_pids/watch_dog_1.pid') is False:
				os.system('python %s/watch_dog1.py start'%self.working_dir)
			
			if self.check_file(self.working_dir + '/daemon_pids/wd2_wd1.wd') is False:
				os.system('python %s/watch_dog1.py restart'%self.working_dir)
...
if __name__ == '__main__':
	daemon = watch_dog1(pidfile=self.working_dir + '/daemon_pids/watch_dog_2.pid')
	if len(sys.argv) == 2:
		if 'start' == sys.argv[1]:
			daemon.start()
